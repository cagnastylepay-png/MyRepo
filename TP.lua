...
local function loadMatrix()
    if isfile(fileName) then
        ...
        if success then
            ...
            Debug("Matrix chargée depuis le fichier JSON.")
            return
        end
    end
    collectionMatrix = {}
    Debug("Nouvelle Matrix initialisée.")
end
...
-- [Logique d'Achat]
local function VerifyCanRebirth()
    local reqs = getNextRebirthRequirements()
    if not reqs then 
        -- Debug("⚠️ Impossible de charger les pré-requis de Renaissance.")
        return false 
    end

    local missingChars = getMissingCharacters()
    local hasAllAnimals = (#missingChars == 0)
    local hasEnoughCash = (playerCash.Value >= reqs.Cash)

    if hasAllAnimals and hasEnoughCash then
        Debug("🎉 TOUT EST PRÊT : Renaissance disponible !")
        return true
    else
        -- Optionnel : Savoir pourquoi ça bloque
        if not hasAllAnimals then Debug("⏳ Manque encore des animaux...") end
        if not hasEnoughCash then Debug("💸 Manque d'argent pour Renaissance.") end
        
        return false -- Très important pour éviter les erreurs de comparaison
    end
end

local function buyConditionValidation(price, name, income, rarity, mutation)
    -- Debug initial pour voir ce qui entre dans la fonction
    Debug(string.format("🧐 [CHECK]: %s | Price: %d | Inc: %d | Rar: %s | Mut: %s", name, price, income, rarity, mutation))

    if playerCash.Value < price then 
        Debug("❌ [REFUS]: Pas assez de cash (" .. playerCash.Value .. " < " .. price .. ")")
        return false 
    end
    ...
    -- Debug des stats du terrain
    Debug(string.format("📊 [STATS]: Slots %d/%d (Invest Max: %d) | New: %s | Rebirth: %s", 
        currentCount, totalSlots, maxInvestSlots, tostring(isNewForCollection), tostring(isRequiredForRebirth ~= nil)))
    ...
    if mode == "IndexAndRebirth" then
        if isRequiredForRebirth and currentCount < totalSlots then
            Debug("✅ [ACHAT]: Requis pour Rebirth")
            return true
        end
    
        if isNewForCollection and currentCount < maxInvestSlots then
            Debug("✅ [ACHAT]: Nouveau dans la collection")
            return true
        end
    end

    Debug("❌ [REFUS]: Ne remplit aucune condition")
    return false
end
...
task.spawn(function()
    while true do
        if isStarted then
            ...
            -- 2. Anti-pousse (Retour intelligent)
            -- On ne force le retour que si on n'est pas déjà en train de bouger (Collecte)
            local dist = (rootPart.Position - purchasePosition).Magnitude
            if dist > 5 then
                -- On vérifie que le personnage ne bouge pas déjà vers un podium
                -- pour éviter que le bot "tremble" entre deux positions
        ...
        task.wait(1) -- Fréquence de rafraîchissement
    end
end)
